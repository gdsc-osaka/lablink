...
export async function getCommonAvailability(
  ...
  timeMin: string,
  timeMax: string
): Promise<any> { // Geminiからの応答を受け取るため、返り値の型はanyや適切な型に変更
  if (!accessToken) {
  ...
  try {
    ...
    Object.values(calendarsBusyInfo).forEach(calendarInfo => {
      if (calendarInfo.busy) {
        const validBusySlots = calendarInfo.busy.filter(
          (period): period is TimeSlot => !!period.start && !!period.end
        );
    ...
    return { success: true, message: "Formatted text generated.", data: formattedFreeSlots };

  } catch (error) {
    console.error('Error in getCommonAvailability:', error);
    throw new Error('An error occurred while processing calendar availability.');
  }
}