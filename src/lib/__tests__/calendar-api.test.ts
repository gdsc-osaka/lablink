...
describe('Calendar API Integration', () => {
    ...
    const mockCalendarResponse = {
      data: {
        calendars: {
          ...
          'user2@example.com': {
            busy: [
              { start: '2024-01-01T21:00:00Z', end: '2024-01-01T23:00:00Z' }
            ]
          }
        }
      }
  ...
  it('APIエラーが発生した場合、適切にエラーハンドリングされる', async () => {
    ...
    await expect(
      getCommonAvailability(mockAccessToken, mockUserEmails, mockTimeMin, mockTimeMax)
    ).rejects.toThrow('An error occurred while processing calendar availability.');
  });
        ...
        calendars: {
          'user1@example.com': {
            busy: [
              ...
              { start: '2024-01-01T15:00:00Z', end: '2024-01-01T16:00:00Z' }
            ]
          }
        }
      }
