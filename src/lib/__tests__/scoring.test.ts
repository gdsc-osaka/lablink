...
describe('splitInto30MinSlots', () => {
    it('should generate slots starting at 00 minutes only', () => {
        const freeSlots = [
            { start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T02:00:00.000Z' }
        ];
    ...
    it('should round up non-00 minute starts to next hour', () => {
        const freeSlots = [
            { start: '2025-01-01T00:15:00.000Z', end: '2025-01-01T02:00:00.000Z' }
        ];
        const result = splitInto30MinSlots(freeSlots);
    ...
    it('should handle already aligned 00 minute starts', () => {
        const freeSlots = [
            { start: '2025-01-01T03:00:00.123Z', end: '2025-01-01T04:00:00.000Z' }
        ];
        const result = splitInto30MinSlots(freeSlots);
    ...
    it('should return empty array for slots too short to fit 30 minutes', () => {
        const freeSlots = [
            { start: '2025-01-01T00:15:00.000Z', end: '2025-01-01T00:45:00.000Z' }
        ];
        const result = splitInto30MinSlots(freeSlots);
    ...
    it('should handle multiple free slots', () => {
        const freeSlots = [
            { start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T01:00:00.000Z' },
            { start: '2025-01-01T02:00:00.000Z', end: '2025-01-01T03:00:00.000Z' }
        ];
        const result = splitInto30MinSlots(freeSlots);
...
describe('filterByTimeOfDay', () => {
    it('should filter morning slots (6-12 JST)', () => {
        const candidates = [
            { start: '2025-01-01T21:00:00.000Z', end: '2025-01-01T23:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }, // JST 6:00 (morning)
            { start: '2025-01-01T07:00:00.000Z', end: '2025-01-01T09:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }  // JST 16:00 (evening)
        ];
    ...
    it('should filter noon slots (12-16 JST)', () => {
        const candidates = [
            { start: '2025-01-01T03:00:00.000Z', end: '2025-01-01T05:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }, // JST 12:00 (noon)
            { start: '2025-01-01T21:00:00.000Z', end: '2025-01-01T23:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }  // JST 6:00 (morning)
        ];
    ...
    it('should filter night slots (19+ or <6 JST)', () => {
        const candidates = [
            { start: '2025-01-01T10:00:00.000Z', end: '2025-01-01T12:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }, // JST 19:00 (night)
            { start: '2025-01-01T19:00:00.000Z', end: '2025-01-01T21:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }, // JST 4:00 (night)
            { start: '2025-01-01T21:00:00.000Z', end: '2025-01-01T23:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 }  // JST 6:00 (morning)
        ];
    ...
    it('should handle timezone boundary correctly (23:00 UTC + 9 = 8:00 JST next day)', () => {
        const candidates = [
            { start: '2025-01-01T23:00:00.000Z', end: '2025-01-02T01:00:00.000Z', totalScore: 10, requiredMemberCount: 1, optionalMemberCount: 0 } // JST 8:00 (morning)
        ];
...
describe('generateCandidates', () => {
    it('should generate candidates with 1-hour stepping', () => {
        const slots = [
            { start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T00:30:00.000Z', availableMembers: ['user1'], score: 10 },
            { start: '2025-01-01T00:30:00.000Z', end: '2025-01-01T01:00:00.000Z', availableMembers: ['user1'], score: 10 },
            { start: '2025-01-01T01:00:00.000Z', end: '2025-01-01T01:30:00.000Z', availableMembers: ['user1'], score: 10 },
            { start: '2025-01-01T01:30:00.000Z', end: '2025-01-01T02:00:00.000Z', availableMembers: ['user1'], score: 10 }
        ];
        const members: MemberInfo[] = [{ userId: 'user1', email: 'test@example.com', isRequired: true }];
        ...
        expect(result.some(c => c.start === '2025-01-01T00:00:00.000Z')).toBe(true);
        expect(result.some(c => c.start === '2025-01-01T01:00:00.000Z')).toBe(true);
    });

    it('should prioritize required members in scoring', () => {
        const slots = [
            { start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T00:30:00.000Z', availableMembers: ['user1', 'user2'], score: 11 },
            { start: '2025-01-01T00:30:00.000Z', end: '2025-01-01T01:00:00.000Z', availableMembers: ['user1', 'user2'], score: 11 }
        ];
        const members: MemberInfo[] = [
            { userId: 'user1', email: 'required@example.com', isRequired: true },
            { userId: 'user2', email: 'optional@example.com', isRequired: false }
        ];
    ...
    it('should sort by required member count then score', () => {
        const slots = [
            { start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T00:30:00.000Z', availableMembers: ['user1', 'user2'], score: 11 },
            { start: '2025-01-01T00:30:00.000Z', end: '2025-01-01T01:00:00.000Z', availableMembers: ['user1', 'user2'], score: 11 },
            { start: '2025-01-01T02:00:00.000Z', end: '2025-01-01T02:30:00.000Z', availableMembers: ['user1'], score: 10 },
            { start: '2025-01-01T02:30:00.000Z', end: '2025-01-01T03:00:00.000Z', availableMembers: ['user1'], score: 10 }
        ];
        const members: MemberInfo[] = [
            { userId: 'user1', email: 'required@example.com', isRequired: true },
            { userId: 'user2', email: 'optional@example.com', isRequired: false }
        ...
        expect(result[0].totalScore).toBeGreaterThanOrEqual(result[1]?.totalScore || 0);
    });
});
...
describe('calculateSlotAvailability', () => {
    it('should calculate available members and scores for each slot', () => {
        ...
        const memberAvailability = new Map([
            ['user1', [{ start: '2025-01-01T00:00:00.000Z', end: '2025-01-01T01:00:00.000Z' }]],
            ['user2', [{ start: '2025-01-01T00:30:00.000Z', end: '2025-01-01T01:00:00.000Z' }]]
        ]);
        const members: MemberInfo[] = [
            { userId: 'user1', email: 'required@example.com', isRequired: true },
            { userId: 'user2', email: 'optional@example.com', isRequired: false }
        ...
        const result = calculateSlotAvailability(slots, memberAvailability, members);
